...
/**
 * POST /api/deals/[id]/ic-package/regenerate
 * Body: { context: DealContext, section: SectionKey }
 * Returns: { prose: Partial<ProseSections> }
 *
 * Used by per-section "Regenerate" buttons in the inline editor. Scoped
 * so a user can redo just the Risk block without disturbing edits to
...
export async function POST(
  ...
  try {
    ...
    if (!section || !VALID_SECTIONS.includes(section as SectionKey)) {
      return NextResponse.json({ error: `Invalid section. Expected one of: ${VALID_SECTIONS.join(", ")}` }, { status: 400 });
    }

    const prose = await regenerateSection(context, section as SectionKey);
    return NextResponse.json({ prose });
  } catch (err) {
    console.error("POST /api/deals/[id]/ic-package/regenerate error:", err);
