...
// Acquisition workspace — the pipeline deep view. Moved here from the root
// home page, which is now the role-based triptych. Every deal whose
// classification includes "acquisition" surfaces here, drag-drop between
// stages, analytics strip, and the optional activity feed side panel.
...
export default function AcquisitionPage() {
  ...
  const handleDrop = async (e: React.DragEvent, newStatus: DealStatus) => {
    ...
    try {
      ...
      setDeals((prev) => prev.map((d) => (d.id === dealId ? { ...d, status: oldStatus } : d)));
      toast.error("Failed to move deal");
    }
  };

  // Only show deals that classify into the Acquisition department. This
  ...
  const filtered = acqDeals.filter((d) => {
  ...
  const activeDeals = acqDeals.filter((d) => !["closed", "dead", "archived"].includes(d.status));
  const dealCost = (d: DealWithStats) =>
    (d.total_project_cost && d.total_project_cost > 0 ? d.total_project_cost : d.asking_price) || 0;
  const pipelineDeals = acqDeals.filter((d) =>
          ...
          <div className="relative max-w-full mx-auto px-6 sm:px-8">
            <div className="flex items-center justify-between h-14 min-w-0">
              <div className="flex items-center gap-2.5">
                <Compass className="h-4 w-4" style={{ color: "hsl(var(--phase-acq))" }} />
                <span className="font-nameplate text-xl leading-none tracking-tight">
                  Acquisition
                </span>
                <span className="text-2xs uppercase tracking-[0.15em] text-muted-foreground/70">
                  Pipeline
                </span>
              </div>
          ...
          <div className="shrink-0 border-b border-border/30 bg-card/30 backdrop-blur-sm animate-fade-up">
            <div className="max-w-full mx-auto px-6 sm:px-8 py-2.5 flex items-center gap-5">
              ...
              <div className="flex items-end gap-2 flex-1 min-w-0">
                {columnMetrics
                  .filter((m) => m.status !== "dead" && m.status !== "archived")
                  .map((m) => {
                    ...
                    return (
                      <div
                        ...
                        title={`${stageLabels[m.status as DealStatus] ?? m.status}: ${formatCurrency(m.value)}`}
                      >
                        <div
                          className={cn("w-full rounded-sm transition-all", colors.dot)}
                          style={{
                            height: `${barHeight}px`,
                            opacity: barHeight > 0 ? 1 : 0.15,
                            minHeight: barHeight > 0 ? undefined : "4px",
                          }}
                        />
          ...
          <main className="flex-1 overflow-x-auto px-6 sm:px-8 py-5">
            {loading ? (
              ...
              <div className="text-center py-32 animate-fade-up">
                ...
                <h2 className="font-nameplate text-3xl mb-2 text-foreground">No deals in acquisition</h2>
                <p className="text-sm text-muted-foreground mb-8 max-w-sm mx-auto">
                  Create your first deal to start building the pipeline.
                </p>
                {can("deals.create") && (
                  <Link href="/deals/new">
                    <Button>
                  ...
                  <div className="divide-y divide-border/20">
                    {activityEvents.map((event, i) => {
                      ...
                      return (
                        <div
                          key={i}
                          className="flex gap-3 px-4 py-3 hover:bg-muted/10 transition-colors"
                        >
                          <div
                            className={cn(
