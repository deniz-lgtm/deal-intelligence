...
export default function AcquisitionScheduleEditor({ dealId }: { dealId: string }) {
  const [importOpen, setImportOpen] = useState(false);
  const [scheduleKey, setScheduleKey] = useState(0);

  return (
    <div className="space-y-8">
      <OtherTrackLinks dealId={dealId} current="acquisition" />
      <div>
        <div className="mb-6 flex items-start justify-between gap-4">
          ...
          <div className="flex flex-wrap items-center gap-2">
            <Link
              href={`/deals/${dealId}/chat?prompt=${encodeURIComponent("Ask me the key prep questions you need before changing this acquisition schedule. Then identify missing checkpoints, sequencing risks, and handoff items.")}`}
            >
