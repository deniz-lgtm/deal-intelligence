...
import { solveMaxBid, type CalcMode, type MaxBidTargets } from "@/lib/max-bid";
            ...
            <div className="rounded-lg border bg-gradient-to-br from-primary/10 to-primary/5 p-5">
              <div className="flex items-baseline justify-between gap-4 flex-wrap">
                <div>
                  ...
                  <div className="text-3xl font-bold tabular-nums mt-1">{zeroBid ? "Deal fails at any price" : fc(result.max_bid)}</div>
              ...
              {zeroBid && (
                <div className="mt-3 text-xs text-red-400 flex items-start gap-2">
                  ...
                  <span>
                    Even at zero {data.development_mode ? "land cost" : "basis"}, the deal fails the <strong className="font-semibold">{result.binding_constraint.replace("_", " ")}</strong> hurdle. Revisit OpEx, rents, or debt structure — no price will save it.
                  </span>
                </div>
              )}
            </div>
